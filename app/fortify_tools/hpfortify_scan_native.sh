...
REPORT_NAME="JNI_EXAMPLE"_$(date +'%Y%m%d%H%M%S')

WORKING_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
BUILD_DIR=${WORKING_DIR}/../build-jni
FPR="$BUILD_DIR/$REPORT_NAME.fpr"

# Following exports need to be configured according to host machine.
export ANDROID_SDK_HOME=/Users/myname/Library/Android/sdk
...
# E.g. JniExample/app/build-jni/CMakeFiles/3.10.2
export CMAKE_FILES_PATH=${WORKING_DIR}/../build-jni/CMakeFiles/$CMAKE_VERSION
...
function configure_cmake_files {
    ...
    $ANDROID_CMAKE_HOME/cmake -H$BUILD_DIR/../ \
        -DCMAKE_CXX_FLAGS=-std=c++11 -frtti -fexceptions \
        -DCMAKE_FIND_ROOT_PATH=$BUILD_DIR/.cxx/cmake/release/prefab/x86/prefab \
        ...
        -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$BUILD_DIR/intermediates/cmake/release/obj/x86 \
        -DCMAKE_MAKE_PROGRAM=$ANDROID_SDK_HOME/cmake/$CMAKE_VERSION_PATH/bin/ninja \
        -DCMAKE_SYSTEM_NAME=Android \
        -DCMAKE_SYSTEM_VERSION=16 \
        -B$BUILD_DIR/.cxx/cmake/release/x86 \
        -GNinja ..
}
...
function cleanup {
    rm -rf $BUILD_DIR/CMakeFiles/native-lib.dir
    rm -rf $FPR
}
...
function replace_compiler_paths {
    ...
    sed -i '' 's+'$CLANGXX_PATH.*[^")"]'+'$HPFORTIFY_CXXPATH'+g' $CMAKE_FILES_PATH/CMakeCXXCompiler.cmake
}
...
echo "[========Clean up the build intermediates and the older fpr file ========"]
cleanup
